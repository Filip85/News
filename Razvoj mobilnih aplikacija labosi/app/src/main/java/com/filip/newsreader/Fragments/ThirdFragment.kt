...
class ThirdFragment() : Fragment() {
    ...
    fun redTextFromUrl(){
        doAsync {                   //služi kako se ne bi odvijalo na glavnoj nitim, ako se to ne korsti crashat će se aplikacija
            val doc = Jsoup.connect(NewsDatabase.newsDao.getArticle(2)).get() //stvaramo konekciju sa url-um te u varijablu doc trpamo cijeli HTML sadržaj pomoću metode get
    ...
    fun showTextOfArticle(show: Elements){
        articleTitle3.text = NewsDatabase.newsDao.getTitleOfArticle(2).toString()
        articleText3.text = show.text()    //prikaz članka odnosno samo pragrafa web stranice
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {   //picasso mi nije radio unutart showTextOfArticle pa sam ga stavio ovdje
        ...
        Picasso.get()
                .load(NewsDatabase.newsDao.getImageOfArticle(2))
                .into(articleImage3)
    }
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {   //morao sam staviti postavljanje naslova u setUserVisibleHint kako bi se na swap promjenio naslov
        ...
        if (isVisibleToUser) {
            Handler().postDelayed({
                (activity as AppCompatActivity).supportActionBar?.title = NewsDatabase.newsDao.getTitleOfArticle(2)
