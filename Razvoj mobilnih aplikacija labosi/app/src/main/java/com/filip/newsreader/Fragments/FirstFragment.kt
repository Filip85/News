...
class FirstFragment(): Fragment() {
    ...
    fun redTextFromUrl(){
        doAsync {                   //služi kako se ne bi odvijalo na glavnoj nitim, ako se to ne korsti crashat će se aplikacija
            val doc = Jsoup.connect(NewsDatabase.newsDao.getArticle(0)).get() //stvaramo konekciju sa url-um te u varijablu doc trpamo cijeli HTML sadržaj pomoću metode get
    ...
    fun showTextOfArticle(show: Elements){
        articleTitle.text = NewsDatabase.newsDao.getTitleOfArticle(0)
        articleText.text = show.text()    //prikaz članka odnosno samo pragrafa web stranice

        Toast.makeText(MyApplication.ApplicationContext, NewsDatabase.newsDao.getTitleOfArticle(0).toString(), Toast.LENGTH_LONG).show()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {   //picasso mi nije radio unutart showTextOfArticle pa sam ga stavio ovdje
        super.onViewCreated(view, savedInstanceState)                       //odnosno kada se pozove metoda readTextFromUrl koja zove showTextOfArticle ne ucita se slika, picasso se ne će izvršiti ni unutar onCreateView

        Picasso.get()
                .load(NewsDatabase.newsDao.getImageOfArticle(0))
                .into(articleImage)
    }

    /*override fun onResume() {
        super.onResume()
        (activity as AppCompatActivity).supportActionBar?.title = NewsDatabase.newsDao.getTitleOfArticle(0)
    }*/
